    ...
    return (
        <ul tabIndex={0} className="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-full place-self-center mt-16 m-4">
            {filteredData!.slice(0, 4).map((item) => (
                <Link to={`/view-manga/${item.slug}`} state={{ previousPage: "Discover" }} className="flex flex-col gap-4 mb-4 w-fit">
                <li key={item.slug} className="h-1/3 w-full border-b-2 flex flex-row py-4">
